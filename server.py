...
def webToBot():
    ...
    if content is None:
        return 'empty input'
    response = requestRasabotServer('UnidtBot', content)
...
def requestRasabotServer(userid, content):
    ...
    reponse = requests.post(rasaUrl,data=json.dumps(params),headers={'Content-Type': 'application/json'})
    return reponse
...
if __name__ == '__main__':
    ...
    # logging.basicConfig(handlers=[fh],level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s',datefmt='%a, %d %b %Y %H:%M:%S',)

    # 启动服务，开启多线程、debug模式
    # 浏览器访问http://127.0.0.1:8088/ai?content="你好"
    app.run(host=webIp, port=int(webPort), threaded=True, debug=True)
