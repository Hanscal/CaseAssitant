...
# graph = Graph("http://CHneo4j:neo4j@127.0.0.1:7474")
...
#     body = {"statements": [{"statement": cyber}]}
#     headers = {'content-type': "application/json; charset=utf-8"}
#     response = requests.post(url, data=str(json.dumps(body,separators=(',',';'))), headers=headers)
...
if __name__ == '__main__':
    selector = NodeMatcher(graph)
    all_defendants = []
    cyber = """MATCH (n) WHERE n.`案件号` ENDS WITH "浙1125刑初148号" return n"""
    # a = list(selector.match("被告人", 案件号__endswith="浙1125刑初148号"))
    a = list(graph.run(cyber))
    for _ in a:
        all_defendants.append(_['n']['name'])
    set_def = set(all_defendants)
    print(','.join(list(set_def)))

    retrieveDataFromNeo4j(cyber)