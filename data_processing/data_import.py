...
# graph = Graph("http://CHneo4j:neo4j@0127.0.0.1:7474")
...
for relationship_id, relationship in relationship_dict.items():
    ...
    query1 = "MATCH (m:{}) where m.id='{}' return m".format(start_node_labels[0],start_node_id)
    query2 = "MATCH (m:{}) where m.id='{}' return m".format(end_node_labels[0],end_node_id)
    existing_u1 = graph.run(query1)
    existing_u2 = graph.run(query2)
